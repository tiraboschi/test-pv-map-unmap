...
# Name of the storage class to be used for the test, empty will choose then default one
STORAGECLASS=${STORAGECLASS:-""}
# Storage request for test PVCs
STORAGEREQUEST=${STORAGEREQUEST:-"1Gi"}
# Number of seconds of delay after the creation of each job
RAMPUPDELAYSECONDS=${RAMPUPDELAYSECONDS:-"1"}
...
for instance in $(seq -w 0001 "$NUMPARALLEL"); do
  sed "s/{{INSTANCE}}/$instance/g" sequence-job.yaml | \
  sed "s/{{NAMESPACE}}/$NAMESPACE/g" | \
  sed "s/{{SLEEPSECONDS}}/$SLEEPSECONDS/g" | \
  sed "s/{{MINWRITESECONDS}}/$MINWRITESECONDS/g" | \
  sed "s/{{MAXWRITESECONDS}}/$MAXWRITESECONDS/g" | \
  sed "s/{{PVCACCESSMODE}}/$PVCACCESSMODE/g" | \
  sed "s/{{ITERATIONS}}/$ITERATIONS/g" | \
  sed "s/{{STORAGECLASS}}/$STORAGECLASS/g" | \
  sed "s/{{STORAGEREQUEST}}/$STORAGEREQUEST/g" | \
  oc apply -n "${NAMESPACE}" -f -
  sleep "${RAMPUPDELAYSECONDS}"
done
